...
function extractTitle(html: string, fallback: string): string {
  ...
  const title = t ? decodeHtmlEntities(t[1]).trim() : ''
...
interface RawPage { segments: string[]; html: string }
 ...
 */
function collectHtml(zip: AdmZip, out: { entryName: string; html: string }[], depth = 0): void {
  if (depth > 4) return
  for (const entry of zip.getEntries()) {
    if (entry.isDirectory) continue
    if (/\.zip$/i.test(entry.entryName)) {
      try { collectHtml(new AdmZip(entry.getData()), out, depth + 1) } catch { /* skip bad nested zip */ }
      continue
    }
    if (!/\.html$/i.test(entry.entryName)) continue   // skip .csv (databases), images, assets
    out.push({ entryName: entry.entryName, html: entry.getData().toString('utf-8') })
  }
}

export function parseNotionZip(zipPath: string): ExternalNote[] {
  const raw: { entryName: string; html: string }[] = []
  collectHtml(new AdmZip(zipPath), raw)
